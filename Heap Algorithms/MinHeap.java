...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	public Decreaser<T> insert(T thing) {
		...
		int x = size;
		ticker.tick();
		while((x / 2) > 0 && array[x].getValue().compareTo(array[x/2].getValue()) < 0){
			Decreaser<T> storage = array[x];
			ticker.tick();
			array[x] = array[x / 2];
			ticker.tick();
			array[x / 2] = storage;
	...
	private void heapify(int where) {
		...
		else if(array[where].getValue().compareTo((array[where * 2].getValue())) > 0 && ((where*2) + 1 > size || array[(where * 2) + 1] == null || array[where * 2].getValue().compareTo((array[(where * 2) + 1].getValue())) <= 0)){
			// parent where is greater than left child and left child is less than right
			Decreaser<T> storage = array[where];
			ticker.tick();
			array[where] = array[where * 2];
			ticker.tick();
			array[where * 2] = storage;
			ticker.tick();
		...
		else if((where * 2) + 1 > size || array[(where * 2) + 1] == null){
			
		}
		else if(array[where].getValue().compareTo((array[(where * 2) + 1].getValue())) > 0 && array[where * 2].getValue().compareTo((array[(where * 2) + 1].getValue())) >= 0){
			// parent where is greater than left child and left child is less than right
			Decreaser<T> storage = array[where];
			ticker.tick();
			array[where] = array[(where * 2) + 1];
			ticker.tick();
			array[(where * 2) + 1] = storage;
			ticker.tick();
			array[where].loc = where;
			ticker.tick();
